...
(define-test-suite pruebas
  ...
  (test-case "begin-exp"
             (check-equal? (run `(begin
                                   (newref 9) ((emptylist null))))
                           (computation (ref-val 0) (list  (num-val 9))))
             (check-equal? (run `(begin
                                   (newref 9) ((deref 0))))
                           (computation (num-val 9) (list (num-val 9))))
             (check-equal? (run `(begin
                                   (newref 9) ((newref 5) (newref 6) (- 7 1))))
                           (computation (num-val 6) (list (num-val 9) (num-val 5) (num-val 6))))
             (check-equal? (run `(begin
                                   (newref 9) ((newref 5) (newref 6) (- 7 1) (emptylist null))))
                           (computation (num-val 6) (list (num-val 9) (num-val 5) (num-val 6)))))
