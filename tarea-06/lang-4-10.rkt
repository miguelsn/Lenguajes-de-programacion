...
(define (parse-begin x)
  ...
  (begin-exp (parse-expression (second x)) (parse-expressions (third x))))
  ...
  (cond
    ...
    [(begin-exp? exp)
     (let* ([exp1 (begin-exp-exp1 exp)]
            ...
            [val1 (value-of exp1 env s)])
       (if (or
            (null? exps)
            (emptylist? (first exps)))
           val1
           (value-of (begin-exp (first exps) (rest exps)) env (computation-store val1))))]
    [(emptylist? exp)
     (emptylist-null exp)]
[else
