...
(struct store (vector head)
   #:transparent)

;; empty-store : () -> Sto
(define (empty-store)
  (store (make-vector 10) 0))
...
(define (extend-store ostore)
  (let* ([store-size (vector-length (store-vector ostore))]
         [nstore (store (make-vector (+ store-size 10)) (store-head ostore))])
    (let loop ([i 0])
      (if (< i store-size)
          (let ([val (vector-ref (store-vector ostore) i)])
            (vector-set! (store-vector nstore) i val)
            (loop (+ i 1)))
          nstore))))
;;;;;;;;;;;;;;;;;;
;; COMPUTATION  ;;
  ...
  (cond
    ...
    [(newref-exp? exp)
     (let* ([exp1 (newref-exp-exp1 exp)]
            [next-ref (store-head the-store)]
            [cmp1 (value-of exp1 env s) ]
            [val1 (computation-val cmp1)]
            [s1 (computation-store cmp1)])
       (if (eq? (store-head the-store) (vector-length (store-vector the-store)))
           (let ([nstore (extend-store the-store)])
             (vector-set! (store-vector nstore) (store-head nstore) val1)
             (set! the-store (store (store-vector nstore) (+ (store-head nstore) 1))))
           (let ([nstore (store (store-vector the-store) (+ (store-head the-store) 1))])
             (vector-set! (store-vector nstore) (store-head the-store) val1)
             (set! the-store nstore)))
    ...
    [(deref-exp? exp)
     (let* ([exp1 (deref-exp-exp1 exp)]
       ...
       (if (or
            (empty? (get-store))
            (< loc 0)
            (<= (- (vector-length (store-vector the-store)) 1) loc))
           (error 'deref-exp "no se puede encontrar la locación ~e" loc)
           (computation (vector-ref (store-vector the-store) loc) the-store)))]
    [(setref-exp? exp)
     (let* ([exp1 (setref-exp-exp1 exp)]
            ...
            [loc (expval->num val1)])
             (if (or
                  (empty? (get-store))
                  (< loc 0)
                  (<= (vector-length (store-vector the-store)) loc))
                 (error 'deref-exp "no se puede encontrar la locación ~e" loc)
                 (begin
                   (vector-set! (store-vector the-store) loc val2)
